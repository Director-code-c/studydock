...
import { updateCourseAction } from "@/components/courses/course-actions"
...
function EditCourseForm({ course, onClose }: { course: Course; onClose: () => void }) {
  const [selectedColor, setSelectedColor] = useState(course.color ?? "")
  const [state, formAction, pending] = useActionState(updateCourseAction, {})
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        {open ? <EditCourseForm key={course.id} course={course} onClose={() => setOpen(false)} /> : null}
      </DialogContent>
    </Dialog>
  )
}
